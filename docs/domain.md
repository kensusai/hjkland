...
| パー                        | `par`                                  | お題ごとの基準キーストローク数。スタンプ判定の基準。ゴルフ用語に由来。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| キーストローク              | `keystroke`                            | プレイヤーが押した1キー入力。スコアの計測単位。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 試行                        | `Attempt`                              | お題への1回の挑戦。キーストローク列・所要時間・結果(クリア/放棄)を記録する。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| スタンプ(旧: メダル)        | `Medal` (`gold` / `silver` / `bronze`) | クリア時の評価。遊園地のスタンプラリーに見立てる。パー以内=金、パー×1.5以内=銀、クリア=銅。テーマパーク統一(2026-08-01)で表示名をメダルからスタンプに変更。コード名 `Medal` は保存データ互換のため据え置き(`Belt` と同様)。                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 学習活動                    | `LearningActivity`                     | ストリーク継続の条件となる活動。レッスンクリア・ドリルセッション完了・デイリーチャレンジクリア・**コマンドクイズ完了(スマホ向け)**のいずれか。クイズはタップ式の認識ドリルで、ストリークは維持するが XP・スタンプは付かない。                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 実績                        | `Achievement`                          | 条件達成で解除されるバッジ(例: 初の金スタンプ、ストリーク30日)。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 弱点コマンド                | `WeakCommand`                          | 成績データから「苦手」と判定されたコマンド。ドリルで再出題の重みが上がる。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 進捗                        | `Progress`                             | プレイヤーの永続データ全体。含むもの: レッスンクリア状況、解放済みコマンド、お題ごとのベストスタンプ・ベストキーストローク、試行履歴(習熟度・弱点分析の元データ)、XP・レベル・実績、ストリーク・フリーズ在庫。すべてブラウザローカルに自動保存され、明示的な「セーブ操作」は不要。                                                                                                                                                                                                                                                                                                                                                                           |
...
| UC2 | プレイヤー   | ドリルで解放済みコマンドを反復し、スタンプを狙う                    |
...
- R3: スタンプ判定: `keystrokes <= par` なら金、`keystrokes <= ceil(par * 1.5)` なら銀、クリアすれば銅。
- R4: 同一お題を再挑戦した場合、スタンプはベスト記録で上書きされる(下がらない)。スタンプ閾値は難易度に依存するため、スタンプとベストキーストロークはそれぞれ独立に単調改善する(難易度を変えた再挑戦でどちらか一方だけが更新されることがある)。
...
- R15: 同じ日のデイリーチャレンジは何度でも再挑戦できるが、XP が付与されるのは初回クリアのみ(スタンプのベスト更新は可)。
...
- R19: 弱点判定は試行履歴(お題に使われたコマンドとスタンプ結果)から計算する。弱点コマンドはドリル生成時の出題重みが上がる。

## 例外ケース

| ケース                            | 業務的な扱い                                                                                                |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| お題を途中で放棄(リタイア)        | 試行は「放棄」として記録。スタンプ・XP なし。弱点分析の入力にはなる。                                       |
...
- ストリーク・実績・スタンプを遡って不利に改変しない(データ修復を除く)。
...
| #   | 論点                      | 暫定判断                                                                                                                                                                                                                                                                                    | 根拠                                                                                                                                              |
| --- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1  | XP の具体量               | レッスンクリア 20 / お題クリア(レッスン内・ドリル共通)金10・銀7・銅5 / デイリーチャレンジ初回クリア 15(金スタンプ時はボーナス+5)。※レッスン1周 = お題XP+20(オーナー確認済み 2026-07-11)。実装: `core/progression/xp.ts`                                                                     | Duolingo 等の相場感。バランスは実装後に調整                                                                                                       |
...
| P12 | 難易度設定                | 全体設定1つ(やさしい/ふつう/むずい)。スタンプ基準(金=パー×係数)と補助(ヒント・答え合わせ・入力キー表示)を調整するだけ。クリア可否は変えない=どの難易度でもストリークは安全。実装: `core/difficulty.ts`。設定は UI 設定(localStorage)で Profile には保存しない                               | 両端(退屈な上級者/つまずく初心者)に効く。当初はエリア別解禁案もあったが全体設定1つから開始(オーナー選択)                                          |
