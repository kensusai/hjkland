...
このドキュメントは hjkland のユビキタス言語と業務ルールを定義する。**コード・テスト・ドキュメント・UI 文言は、ここで定義した用語(と対応するコード名)を一貫して使うこと。** 前提となる要件は `docs/project-overview.md` を参照。
...
| カリキュラム                | `Curriculum`                           | 学習内容の全体構成。8つのエリアからなる。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| エリア(旧: ステージ)        | `Area`                                 | カリキュラムの大区分。遊園地の園内区画に見立てる。全8エリア(ゴール=実務の vim が一通り操作できる)。①基本移動・モード操作 ②編集の型(d/c/y×テキストオブジェクト・.) ③検索と置換(:s/:%s/:g) ④レジスタの奥義 ⑤効率化の型(A/I/o/O・r/R・J・ct)/dt,・~) ⑥検索の達人(/? n/N ・* # ・; ,) ⑦ビジュアルの型(v/V/Ctrl-v・gU/gu 等) ⑧実戦総合(全技を使う実務シナリオ総まとめ。旧: 実戦 皆伝)。各エリアの表示名は遊園地の区画名(エントランス広場〜グランドパレード、`src/core/curriculum/areas.ts` が正)。当初4→8に拡張(2026-07-12)。マクロは対象外(オーナー決定)。複数ファイルはエンジン単一バッファ前提(ADR-0006)のためレッスンでは解説・豆知識に留め、ドリル化しない。 |
| レッスン                    | `Lesson`                               | エリア内の学習単位。新コマンドの解説と演習お題のセット。クリアでコマンドが解放される。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| コマンド                    | `VimCommand`                           | 学習対象の vim 操作(例: `w`, `diw`, `:s`)。レッスンにより「解放」される。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 解放                        | `unlock` / `UnlockedCommand`           | レッスンクリアによりコマンドがドリル・デイリーチャレンジの出題対象になること。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| お題                        | `Exercise`                             | 演習課題の最小単位。開始バッファ・目標バッファ・お手本キー数で構成される。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 開始バッファ / 目標バッファ | `initialBuffer` / `targetBuffer`       | お題の初期テキストと、一致すればクリアとなる目標テキスト。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| お手本(旧: パー)            | `par`                                  | お題ごとの基準キーストローク数(作者の模範解答の手数)。ライド評価の基準。お手本どおりの手数でパーフェクト。旧称パーはゴルフ用語で伝わりにくいため表示名を変更(2026-08-01)。コード名 `par` は保存データ互換のため据え置き。                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| キーストローク              | `keystroke`                            | プレイヤーが押した1キー入力。スコアの計測単位。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 試行                        | `Attempt`                              | お題への1回の挑戦。キーストローク列・所要時間・結果(クリア/放棄)を記録する。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ライド評価(旧: メダル)      | `Medal` (`gold` / `silver` / `bronze`) | クリア時の評価。アイコンは使わず評価ワードだけで表示する(疾走感優先、オーナー決定 2026-08-01): お手本以内=パーフェクトライド!!(`gold`)、お手本×1.5以内=ナイスライド!(`silver`)、クリア=完走(`bronze`)。一覧・記録では短縮形(パーフェクト/ナイス/完走)。コード名 `Medal` は保存データ互換のため据え置き(`Belt` と同様)。                                                                                                                                                                                                                                                                                                                                      |
...
| 学習活動                    | `LearningActivity`                     | ストリーク継続の条件となる活動。レッスンクリア・ドリルセッション完了・デイリーチャレンジクリア・**コマンドクイズ完了(スマホ向け)**のいずれか。クイズはタップ式の認識ドリルで、ストリークは維持するが XP・評価は付かない。                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| パスポート(旧: 帯)          | `Belt`                                 | レベルの表示上の呼び名(ワンデーパス→レジェンドパス)。レベルから純関数で導出されるフレーバーで、業務ルールへの影響はない。hjkland リブランド(2026-07-25)で表示名を帯からパスポートに変更。コード名 `Belt` は保存データ互換のため据え置き。                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 実績                        | `Achievement`                          | 条件達成で解除されるバッジ(例: 初のパーフェクトライド、ストリーク30日。UI では実績一覧を「スタンプ帳」として朱印の判子風に表示する)。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 弱点コマンド                | `WeakCommand`                          | 成績データから「苦手」と判定されたコマンド。ドリルで再出題の重みが上がる。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 進捗                        | `Progress`                             | プレイヤーの永続データ全体。含むもの: レッスンクリア状況、解放済みコマンド、お題ごとのベスト評価・ベストキーストローク、試行履歴(習熟度・弱点分析の元データ)、XP・レベル・実績、ストリーク・フリーズ在庫。すべてブラウザローカルに自動保存され、明示的な「セーブ操作」は不要。                                                                                                                                                                                                                                                                                                                                                                               |
...
| UC2 | プレイヤー   | ドリルで解放済みコマンドを反復し、パーフェクトを狙う                |
...
- R3: ライド評価の判定: `keystrokes <= par` ならパーフェクト(`gold`)、`keystrokes <= ceil(par * 1.5)` ならナイス(`silver`)、クリアすれば完走(`bronze`)。
- R4: 同一お題を再挑戦した場合、評価はベスト記録で上書きされる(下がらない)。評価閾値は難易度に依存するため、評価とベストキーストロークはそれぞれ独立に単調改善する(難易度を変えた再挑戦でどちらか一方だけが更新されることがある)。
...
- R15: 同じ日のデイリーチャレンジは何度でも再挑戦できるが、XP が付与されるのは初回クリアのみ(評価のベスト更新は可)。
...
- R19: 弱点判定は試行履歴(お題に使われたコマンドと評価結果)から計算する。弱点コマンドはドリル生成時の出題重みが上がる。

## 例外ケース

| ケース                            | 業務的な扱い                                                                                                        |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| お題を途中で放棄(リタイア)        | 試行は「放棄」として記録。評価・XP なし。弱点分析の入力にはなる。                                                   |
| 生成されたお題が自明すぎる/解なし | 生成器はお手本キー数の下限(例: 3キー以上)を保証し、`initialBuffer == targetBuffer` となるお題を生成してはならない。 |
...
- ストリーク・実績・評価を遡って不利に改変しない(データ修復を除く)。
...
| #   | 論点                      | 暫定判断                                                                                                                                                                                                                                                                                    | 根拠                                                                                                                                              |
| --- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1  | XP の具体量               | レッスンクリア 20 / お題クリア(レッスン内・ドリル共通)パーフェクト10・ナイス7・完走5 / デイリーチャレンジ初回クリア 15(パーフェクト時はボーナス+5)。※レッスン1周 = お題XP+20(オーナー確認済み 2026-07-11)。実装: `core/progression/xp.ts`                                                   | Duolingo 等の相場感。バランスは実装後に調整                                                                                                       |
...
| P4  | お手本の決め方            | 生成器が知る最適解のキーストローク数をお手本とする。手作りお題は作者が最適解を実演して設定                                                                                                                                                                                                  | お手本が理論最適である保証は不要。ナイス・完走で救済される                                                                                        |
| P5  | 弱点判定の閾値            | 直近5試行のうちナイス以上が2回以下のコマンドを弱点とする(試行3回未満のコマンドは判定対象外 — 情報不足での誤判定を防ぐ)。実装: `core/analytics/weakness.ts`                                                                                                                                  | 少ない試行数で判定できる単純なルールから始める                                                                                                    |
...
| P12 | 難易度設定                | 全体設定1つ(やさしい/ふつう/むずい)。評価基準(パーフェクト=お手本×係数)と補助(ヒント・答え合わせ・入力キー表示)を調整するだけ。クリア可否は変えない=どの難易度でもストリークは安全。実装: `core/difficulty.ts`。設定は UI 設定(localStorage)で Profile には保存しない                       | 両端(退屈な上級者/つまずく初心者)に効く。当初はエリア別解禁案もあったが全体設定1つから開始(オーナー選択)                                          |
...
| カリキュラム(Curriculum)    | エリア・レッスンの構成、コマンド解放の管理               | Area, Lesson, VimCommand, unlock      |
| 出題生成(Generation)        | シードからのお題生成、お手本算出、出題範囲の制約充足     | ExerciseGenerator, seed, par          |
