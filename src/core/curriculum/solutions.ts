/**
 * 模範解答 (author solutions) for authored exercises, keyed by exercise id.
 * Single source of truth: ex() attaches these to the Exercise, the result
 * modal shows them as 答え合わせ, and stageContent.test.ts replays them
 * through the real engine to prove solvability and par accuracy.
 */
export const SOLUTIONS: Record<string, string[]> = {
  "s3-l1-e1": [":s/foo/bar/"],
  "s3-l1-e2": [":s/cat/dog/"],
  "s3-l1-e3": [":s/int/let/"],
  "s3-l1-e4": [":s/foo/baz/"],
  "s3-l1-e5": [":s/TODO/DONE/"],
  "s3-l2-e1": [":s/x/o/g"],
  "s3-l2-e2": [":s/a/b/g"],
  "s3-l2-e3": [":s/no/ok/g"],
  "s3-l2-e4": [":s/1/9/g"],
  "s3-l2-e5": [":s/s/S/g"],
  "s3-l3-e1": [":%s/tmp/out/g"],
  "s3-l3-e2": [":%s/old/new/g"],
  "s3-l3-e3": [":%s/i/n/g"],
  "s3-l3-e4": [":%s/red/blue/g"],
  "s3-l3-e5": [":%s/a/x/g"],
  "s3-l4-e1": [":g/DEBUG/d"],
  "s3-l4-e2": [":g/note/d"],
  "s3-l4-e3": [":g/log/d"],
  "s3-l4-e4": [":g/skip/d"],
  "s3-l4-e5": [":g/TMP/d"],
  "s3-l5-e1": [":%s/foo/bar/g"],
  "s3-l5-e2": [":g/DBG/d"],
  "s3-l5-e3": [":%s/n/k/g"],
  "s3-l5-e4": [":s/aaa/ccc/g"],
  "s4-l1-e1": ['"', "a", "y", "y", "G", '"', "a", "p"],
  "s4-l1-e2": ['"', "a", "y", "y", "j", '"', "a", "p"],
  "s4-l1-e3": ['"', "a", "y", "y", "G", '"', "a", "p"],
  "s4-l1-e4": ['"', "a", "y", "y", "G", '"', "a", "p", '"', "a", "p"],
  "s4-l1-e5": ['"', "a", "y", "y", "G", '"', "a", "p"],
  "s4-l2-e1": [
    '"',
    "a",
    "y",
    "y",
    "j",
    '"',
    "b",
    "y",
    "y",
    "G",
    '"',
    "a",
    "p",
    '"',
    "b",
    "p",
  ],
  "s4-l2-e2": [
    '"',
    "a",
    "y",
    "y",
    "j",
    '"',
    "b",
    "y",
    "y",
    "G",
    '"',
    "b",
    "p",
    '"',
    "a",
    "p",
  ],
  "s4-l2-e3": [
    '"',
    "b",
    "y",
    "y",
    "j",
    '"',
    "a",
    "y",
    "y",
    "G",
    '"',
    "b",
    "p",
    '"',
    "a",
    "p",
  ],
  "s4-l2-e4": ['"', "a", "y", "y", "G", '"', "a", "p", '"', "a", "p"],
  "s4-l2-e5": [
    '"',
    "a",
    "y",
    "y",
    "j",
    '"',
    "b",
    "y",
    "y",
    "G",
    '"',
    "a",
    "p",
    '"',
    "b",
    "p",
  ],
  "s4-l3-e1": ['"', "a", "y", "y", "G", '"', "a", "p", '"', "a", "p"],
  "s4-l3-e2": [
    '"',
    "a",
    "y",
    "y",
    "j",
    '"',
    "b",
    "y",
    "y",
    "G",
    '"',
    "b",
    "p",
    '"',
    "a",
    "p",
  ],
  "s4-l3-e3": [
    '"',
    "a",
    "y",
    "y",
    "G",
    '"',
    "a",
    "p",
    '"',
    "a",
    "p",
    '"',
    "a",
    "p",
  ],
  "s4-l3-e4": ["j", '"', "a", "y", "y", "G", '"', "a", "p"],
  "s2-l1-e1": ["d", "w"],
  "s2-l1-e2": ["w", "d", "w"],
  "s2-l1-e3": ["w", "d", "e"],
  "s2-l1-e4": ["d", "w", "d", "w"],
  "s2-l1-e5": ["w", "d", "e"],
  "s2-l2-e1": ["w", "d", "$"],
  "s2-l2-e2": ["$", "d", "0"],
  "s2-l2-e3": ["w", "d", "$"],
  "s2-l2-e4": ["f", "4", "d", "$"],
  "s2-l2-e5": ["w", "d", "$"],
  "s2-l3-e1": ["w", "d", "i", "w"],
  "s2-l3-e2": ["w", "d", "a", "w"],
  "s2-l3-e3": ["w", "d", "i", "w"],
  "s2-l3-e4": ["w", "w", "d", "a", "w"],
  "s2-l3-e5": ["d", "i", "w"],
  "s2-l4-e1": ["c", "i", "w", "b", "a", "r", "<Esc>"],
  "s2-l4-e2": ["w", "w", "c", "i", "w", "r", "i", "g", "h", "t", "<Esc>"],
  "s2-l4-e3": ["c", "i", "w", "d", "o", "g", "<Esc>"],
  "s2-l4-e4": ["w", "c", "i", "w", "v", "a", "l", "<Esc>"],
  "s2-l4-e5": ["w", "c", "i", "w", "y", "<Esc>"],
  "s2-l5-e1": ["f", '"', "d", "i", '"'],
  "s2-l5-e2": ["f", '"', "c", "i", '"', "y", "o", "<Esc>"],
  "s2-l5-e3": ["f", '"', "c", "i", '"', "b", "<Esc>"],
  "s2-l5-e4": ["f", '"', "d", "i", '"'],
  "s2-l5-e5": ["f", '"', "c", "i", '"', "n", "e", "w", "<Esc>"],
  "s2-l6-e1": ["f", "(", "d", "i", "("],
  "s2-l6-e2": ["f", "(", "c", "i", "(", "y", "<Esc>"],
  "s2-l6-e3": ["f", "(", "d", "i", "("],
  "s2-l6-e4": ["f", "(", "c", "i", "(", "x", "<Esc>"],
  "s2-l6-e5": ["f", "(", "c", "i", "(", "1", "<Esc>"],
  "s2-l7-e1": ["y", "y", "p"],
  "s2-l7-e2": ["y", "y", "p"],
  "s2-l7-e3": ["y", "y", "G", "p"],
  "s2-l7-e4": ["y", "y", "p", "p"],
  "s2-l7-e5": ["j", "y", "y", "p"],
  "s2-l8-e1": ["y", "i", "w", "$", "p"],
  "s2-l8-e2": ["y", "i", "w", "$", "p"],
  "s2-l8-e3": ["y", "i", "w", "$", "p"],
  "s2-l8-e4": ["w", "y", "i", "w", "$", "p"],
  "s2-l8-e5": ["y", "i", "w", "$", "p"],
  "s2-l9-e1": ["d", "w", "."],
  "s2-l9-e2": ["d", "w", ".", "."],
  "s2-l9-e3": ["d", "w", ".", ".", "."],
  "s2-l9-e4": ["x", "."],
  "s2-l9-e5": ["x", ".", "."],
  "s2-l10-e1": [
    "w",
    "c",
    "i",
    "w",
    "n",
    "e",
    "w",
    "<Esc>",
    "f",
    '"',
    "c",
    "i",
    '"',
    "n",
    "e",
    "w",
    "<Esc>",
  ],
  "s2-l10-e2": ["f", "(", "c", "i", "(", "x", "<Esc>"],
  "s2-l10-e3": ["y", "y", "p", "d", "w"],
  "s2-l10-e4": ["f", '"', "d", "i", '"'],
  "s1-l1-e1": ["x"],
  "s1-l1-e2": ["x"],
  "s1-l1-e3": ["x", "x"],
  "s1-l1-e4": ["x", "x", "x"],
  "s1-l1-e5": ["x"],
  "s1-l2-e1": ["l", "x"],
  "s1-l2-e2": ["l", "l", "x"],
  "s1-l2-e3": ["l", "l", "l", "x"],
  "s1-l2-e4": ["l", "x", "l", "x"],
  "s1-l2-e5": ["l", "l", "l", "h", "x"],
  "s1-l3-e1": ["j", "x"],
  "s1-l3-e2": ["j", "j", "x"],
  "s1-l3-e3": ["j", "l", "x"],
  "s1-l3-e4": ["j", "k", "l", "x"],
  "s1-l3-e5": ["j", "j", "x"],
  "s1-l4-e1": ["w", "x"],
  "s1-l4-e2": ["w", "w", "x"],
  "s1-l4-e3": ["w", "w", "w", "x"],
  "s1-l4-e4": ["w", "l", "x"],
  "s1-l4-e5": ["w", "w", "w", "w", "x"],
  "s1-l5-e1": ["w", "b", "x"],
  "s1-l5-e2": ["x", "w", "x"],
  "s1-l5-e3": ["w", "w", "b", "x"],
  "s1-l5-e4": ["w", "w", "b", "b", "x"],
  "s1-l5-e5": ["w", "x"],
  "s1-l6-e1": ["e", "x"],
  "s1-l6-e2": ["e", "e", "x"],
  "s1-l6-e3": ["e", "x", "h", "x"],
  "s1-l6-e4": ["e", "e", "e", "x"],
  "s1-l6-e5": ["e", "x"],
  "s1-l7-e1": ["$", "x"],
  "s1-l7-e2": ["$", "x", "x"],
  "s1-l7-e3": ["$", "x", "0", "x"],
  "s1-l7-e4": ["0", "x"],
  "s1-l7-e5": ["$", "x"],
  "s1-l8-e1": ["f", "X", "x"],
  "s1-l8-e2": ["f", "Y", "x"],
  "s1-l8-e3": ["t", ";", "x"],
  "s1-l8-e4": ["f", "X", "x"],
  "s1-l8-e5": ["f", "=", "x"],
  "s1-l9-e1": ["G", "x"],
  "s1-l9-e2": ["G", "$", "x"],
  "s1-l9-e3": ["G", "g", "g", "x"],
  "s1-l9-e4": ["G", "l", "x"],
  "s1-l9-e5": ["G", "g", "g", "x"],
  "s1-l10-e1": ["i", "h", "<Esc>"],
  "s1-l10-e2": ["$", "a", "!", "<Esc>"],
  "s1-l10-e3": ["l", "i", "i", "<Esc>"],
  "s1-l10-e4": ["i", "#", "<Esc>"],
  "s1-l10-e5": ["$", "a", "s", "<Esc>"],
  "s1-l11-e1": ["o", "s", "e", "c", "o", "n", "d", "<Esc>"],
  "s1-l11-e2": ["O", "f", "i", "r", "s", "t", "<Esc>"],
  "s1-l11-e3": ["o", "e", "n", "d", "<Esc>"],
  "s1-l11-e4": ["o", "2", "<Esc>"],
  "s1-l11-e5": ["O", "#", "<Esc>"],
  "s1-l12-e1": ["d", "d"],
  "s1-l12-e2": ["3", "x"],
  "s1-l12-e3": ["2", "d", "d"],
  "s1-l12-e4": ["4", "x"],
  "s1-l12-e5": ["3", "d", "d"],
  // Stage 7 (visual mode). j-based solutions replay in a real browser only
  // (stageContent.test browserOnly + e2e/drive-stage7.mjs).
  // Stage 5 (効率化の型). R solutions replay in a real browser only.
  "s5-l1-e1": ["A", ";", "<Esc>"],
  "s5-l1-e2": ["I", "#", " ", "<Esc>"],
  "s5-l1-e3": ["A", " ", "=", " ", "0", ";", "<Esc>"],
  "s5-l1-e4": ["I", "#", "#", " ", "<Esc>", "A", " ", "#", "#", "<Esc>"],
  "s5-l2-e1": ["r", "b"],
  "s5-l2-e2": ["$", "r", "9"],
  "s5-l2-e3": ["R", "2", "0", "2", "6", "<Esc>"],
  "s5-l2-e4": ["R", "v", "2", ".", "4", ".", "1", "<Esc>"],
  "s5-l3-e1": ["J"],
  "s5-l3-e2": ["J"],
  "s5-l3-e3": ["J", "J"],
  "s5-l3-e4": ["4", "J"],
  "s5-l4-e1": ["d", "t", ","],
  "s5-l4-e2": ["l", "d", "t", ")"],
  "s5-l4-e3": ["c", "t", "f", "D", "O", "N", "E", " ", "<Esc>"],
  "s5-l4-e4": ["f", "(", "l", "c", "t", ")", "5", "0", "<Esc>"],
  "s5-l5-e1": ["~"],
  "s5-l5-e2": ["4", "~"],
  "s5-l5-e3": ["3", "~"],
  "s5-l5-e4": ["~", "f", "r", "~"],
  // Stage 6 (検索の達人). / and ? solutions replay in a real browser only.
  "s6-l1-e1": ["/", "d", "e", "l", "t", "a", "<CR>", "x"],
  "s6-l1-e2": ["/", "f", "i", "x", "<CR>", "l", "r", "a"],
  "s6-l1-e3": ["/", "d", "d", "d", "<CR>", "d", "d"],
  "s6-l1-e4": ["G", "?", "t", "a", "r", "g", "e", "t", "<CR>", "x"],
  "s6-l2-e1": ["/", "r", "e", "d", "<CR>", "x", "n", "x"],
  "s6-l2-e2": ["/", "t", "m", "p", "<CR>", "x", "n", "x", "n", "x"],
  "s6-l2-e3": ["/", "k", "e", "y", "<CR>", "n", "N", "x"],
  "s6-l2-e4": ["/", "l", "o", "g", "<CR>", "x", "n", "n", "x"],
  "s6-l3-e1": ["*", "x"],
  "s6-l3-e2": ["*", "n", "x"],
  "s6-l3-e3": ["G", "#", "x"],
  "s6-l3-e4": ["*", "x"],
  "s6-l4-e1": ["f", "X", ";", "x"],
  "s6-l4-e2": ["f", "X", ";", ",", "x"],
  "s6-l4-e3": ["f", ",", ";", ";", "x"],
  "s6-l4-e4": ["f", "-", ";", ";", "x"],
  "s6-l5-e1": ["/", "e", "r", "r", "<CR>", "d", "d", "n", "d", "d"],
  "s6-l5-e2": ["*", "r", "F"],
  "s6-l5-e3": ["f", "(", ";", ";", "l", "r", "X"],
  // Stage 3 additions (範囲・条件・空置換)
  "s3-l6-e1": [":2,3s/a/b/"],
  "s3-l6-e2": [":1,2s/tmp/fix/"],
  "s3-l6-e3": [":2,3s/o/x/g"],
  "s3-l7-e1": [":g/x/s/x/z/"],
  "s3-l7-e2": [":g/TODO/s/TODO/WIP/"],
  "s3-l7-e3": [":g/err/s/go/stop/"],
  "s3-l8-e1": [":v/keep/d"],
  "s3-l8-e2": [":v/error/d"],
  "s3-l8-e3": [":v/[0-9]/d"],
  "s3-l9-e1": [":%s/@//g"],
  "s3-l9-e2": [":%s/my_//g"],
  "s3-l9-e3": [":%s/;//g"],
  "s3-l10-e1": [":%s/src/lib/g"],
  "s3-l10-e2": [":%s/'/\"/g"],
  "s3-l10-e3": [":g/#/d"],
  // Stage 4 additions (レジスタ応用)
  "s4-l4-e1": ['"', "a", "d", "d", "G", '"', "a", "p"],
  "s4-l4-e2": ["j", '"', "a", "d", "d", "G", '"', "a", "p"],
  "s4-l4-e3": ['"', "a", "d", "d", '"', "a", "p", '"', "a", "p"],
  "s4-l5-e1": ['"', "a", "y", "y", "j", '"', "A", "y", "y", "G", '"', "a", "p"],
  "s4-l5-e2": [
    '"',
    "a",
    "y",
    "y",
    "j",
    "j",
    '"',
    "A",
    "y",
    "y",
    "G",
    '"',
    "a",
    "p",
  ],
  "s4-l5-e3": ['"', "a", "y", "y", "G", '"', "A", "y", "y", '"', "a", "p"],
  "s4-l6-e1": ["y", "y", "j", "d", "d", '"', "0", "p"],
  "s4-l6-e2": ["y", "y", "j", "2", "d", "d", '"', "0", "p"],
  "s4-l6-e3": ["y", "y", "j", "d", "d", '"', "0", "p"],
  "s4-l7-e1": ["y", "y", "j", '"', "_", "d", "d", "p"],
  "s4-l7-e2": ["y", "y", "j", '"', "_", "d", "d", '"', "_", "d", "d", "p"],
  "s4-l7-e3": [
    "y",
    "y",
    "j",
    '"',
    "_",
    "d",
    "d",
    '"',
    "_",
    "d",
    "d",
    '"',
    "_",
    "d",
    "d",
    "p",
  ],
  "s4-l8-e1": [
    '"',
    "a",
    "y",
    "y",
    "j",
    "j",
    '"',
    "A",
    "y",
    "y",
    "G",
    '"',
    "a",
    "p",
  ],
  "s4-l8-e2": ["y", "y", "j", '"', "_", "d", "d", "G", '"', "0", "p"],
  "s4-l8-e3": [
    '"',
    "a",
    "y",
    "y",
    "j",
    "j",
    '"',
    "A",
    "y",
    "y",
    "G",
    '"',
    "a",
    "p",
  ],
  // Stage 5 additions (C/D・cc・cf/df・indent・swap)
  "s5-l6-e1": ["w", "D"],
  "s5-l6-e2": ["f", "8", "C", "3", "0", "0", "0", "<Esc>"],
  "s5-l6-e3": ["f", "#", "D"],
  "s5-l7-e1": ["c", "c", "n", "e", "w", "<Esc>"],
  "s5-l7-e2": ["j", "c", "c", "o", "k", "<Esc>"],
  "s5-l7-e3": ["S", "f", "i", "n", "a", "l", "<Esc>"],
  "s5-l8-e1": ["d", "f", ","],
  "s5-l8-e2": ["c", "f", "=", "i", "d", "=", "<Esc>"],
  "s5-l8-e3": ["2", "d", "f", ";"],
  "s5-l9-e1": [">", ">"],
  "s5-l9-e2": ["<", "<"],
  "s5-l9-e3": ["2", ">", ">"],
  "s5-l10-e1": ["l", "x", "p"],
  "s5-l10-e2": ["d", "d", "p"],
  "s5-l10-e3": ["f", "i", "x", "p"],
  // Stage 6 additions (F/T・%・{}・^・検索リネーム)
  "s6-l6-e1": ["$", "F", "X", "x"],
  "s6-l6-e2": ["$", "T", "X", "x"],
  "s6-l6-e3": ["$", "F", "X", "x", ";", "x"],
  "s6-l7-e1": ["%", "x"],
  "s6-l7-e2": ["f", "(", "%", "x"],
  "s6-l7-e3": ["f", "(", "%", "x"],
  "s6-l8-e1": ["}", "j", "x"],
  "s6-l8-e2": ["}", "}", "j", "x"],
  "s6-l8-e3": ["G", "{", "{", "j", "x"],
  "s6-l9-e1": ["^", "x"],
  "s6-l9-e2": ["^", "i", "#", "<Esc>"],
  "s6-l9-e3": ["^", "x"],
  "s6-l10-e1": [
    "/",
    "o",
    "l",
    "d",
    "<CR>",
    "c",
    "i",
    "w",
    "n",
    "e",
    "w",
    "<Esc>",
    "n",
    ".",
  ],
  "s6-l10-e2": [
    "/",
    "t",
    "m",
    "p",
    "<CR>",
    "c",
    "i",
    "w",
    "b",
    "u",
    "f",
    "<Esc>",
    "n",
    ".",
    "n",
    ".",
  ],
  "s6-l10-e3": [
    "/",
    "f",
    "i",
    "x",
    "<CR>",
    "n",
    "c",
    "i",
    "w",
    "o",
    "k",
    "<Esc>",
  ],
  // Stage 7 additions (v+object・o・gU/gu・V>・V:s)
  "s7-l6-e1": ["w", "v", "i", "w", "d"],
  "s7-l6-e2": ["f", "(", "v", "i", "(", "d"],
  "s7-l6-e3": ["f", '"', "v", "i", '"', "d"],
  "s7-l7-e1": ["l", "l", "v", "l", "o", "h", "d"],
  "s7-l7-e2": ["l", "l", "l", "v", "l", "l", "o", "h", "h", "d"],
  "s7-l7-e3": ["l", "l", "v", "l", "o", "h", "d"],
  "s7-l8-e1": ["w", "g", "U", "i", "w"],
  "s7-l8-e2": ["g", "u", "i", "w"],
  "s7-l8-e3": ["w", "g", "U", "$"],
  "s7-l9-e1": ["V", "j", ">"],
  "s7-l9-e2": ["V", "j", "j", "<"],
  "s7-l9-e3": ["j", "V", "j", ">"],
  "s7-l10-e1": ["V", "j", ":s/a/b/g"],
  "s7-l10-e2": ["V", "j", ":s/x/y/"],
  "s7-l10-e3": ["j", "V", "j", ":s/foo/bar/"],
  // Stage 8 additions (実戦)
  "s8-l5-e1": ["f", "(", "c", "i", "(", "x", "<Esc>"],
  "s8-l5-e2": ["f", "a", "x", "p"],
  "s8-l5-e3": ["f", "b", "r", "x"],
  "s8-l6-e1": ["f", '"', "c", "i", '"', "p", "r", "o", "d", "<Esc>"],
  "s8-l6-e2": ["A", ",", "<Esc>"],
  "s8-l6-e3": ["e", "i", "e", "<Esc>", "f", '"', "c", "i", '"', "y", "<Esc>"],
  "s8-l7-e1": ["f", ",", ";", "l", "d", "f", ","],
  "s8-l7-e2": ["f", "e", "r", "d"],
  "s8-l7-e3": ["A", ",", "z", "<Esc>"],
  "s8-l8-e1": ["j", ">", ">"],
  "s8-l8-e2": ["j", "V", "j", ">"],
  "s8-l8-e3": ["<", "<", "."],
  "s8-l9-e1": [
    '"',
    "a",
    "y",
    "y",
    "j",
    "j",
    '"',
    "A",
    "y",
    "y",
    "G",
    '"',
    "a",
    "p",
  ],
  "s8-l9-e2": [
    '"',
    "a",
    "y",
    "y",
    "j",
    "j",
    '"',
    "A",
    "y",
    "y",
    "G",
    '"',
    "a",
    "p",
  ],
  "s8-l9-e3": ["j", "2", "d", "d"],
  "s8-l10-e1": ["/", "e", "r", "r", "<CR>", "d", "d"],
  "s8-l10-e2": [":g/trace/d"],
  "s8-l10-e3": [":%s/eror/error/g"],
  // Stage 8 (実戦 皆伝)
  "s8-l1-e1": [":%s/tmp/buf/g"],
  "s8-l1-e2": [":s/x/y/g"],
  "s8-l1-e3": ["c", "i", "w", "s", "u", "m", "<Esc>"],
  "s8-l2-e1": [":g/DEBUG/d"],
  "s8-l2-e2": [":g/^$/d"],
  "s8-l2-e3": ["3", "J"],
  "s8-l3-e1": ["V", "d", "G", "p"],
  "s8-l3-e2": ["y", "y", "p", "$", "c", "i", "w", "t", "w", "o", "<Esc>"],
  "s8-l3-e3": ["~", "w", "~"],
  "s8-l4-e1": [":%s/old/val/g", "g", "g", "d", "d"],
  "s8-l4-e2": ["V", "d", "p", "g", "g", "3", "J"],
  "s8-l4-e3": ['"', "a", "y", "y", "G", '"', "a", "p"],
  "s7-l1-e1": ["v", "l", "l", "d"],
  "s7-l1-e2": ["v", "e", "d", "x"],
  "s7-l1-e3": ["w", "v", "e", "d", "x"],
  "s7-l1-e4": ["f", "[", "v", "f", "]", "d"],
  "s7-l2-e1": ["V", "d"],
  "s7-l2-e2": ["j", "V", "d"],
  "s7-l2-e3": ["V", "j", "d"],
  "s7-l2-e4": ["V", "y", "p"],
  "s7-l3-e1": ["V", "d", "p"],
  "s7-l3-e2": ["V", "d", "G", "p"],
  "s7-l3-e3": ["j", "V", "d", "p"],
  "s7-l3-e4": ["V", "j", "d", "G", "p"],
  "s7-l4-e1": ["<C-v>", "j", "j", "d"],
  "s7-l4-e2": ["<C-v>", "j", "l", "d"],
  "s7-l4-e3": ["<C-v>", "G", "d"],
  "s7-l4-e4": ["<C-v>", "j", "l", "l", "d"],
  "s7-l5-e1": ["<C-v>", "j", "y", "p"],
  "s7-l5-e2": ["<C-v>", "G", "y", "p"],
  "s7-l5-e3": ["<C-v>", "j", "y", "$", "p"],
  "s7-l5-e4": ["<C-v>", "j", "l", "y", "$", "p"],
};
